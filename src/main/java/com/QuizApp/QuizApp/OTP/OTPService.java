package com.QuizApp.QuizApp.OTP;

import okhttp3.OkHttpClient;
import okhttp3.Request;
import okhttp3.Response;
import org.springframework.http.HttpStatus;
import org.springframework.http.ResponseEntity;
import org.springframework.stereotype.Service;

@Service
public class OTPService {
    public ResponseEntity<String> sendOTp(Long phoneNumber) {
        OkHttpClient okHttpClient = new OkHttpClient().newBuilder().build();
        String api_key = "a4db2408-c0b3-11ee-8cbb-0200cd936042";

        String url = "https://2factor.in/API/V1/" + api_key + "/SMS/" + phoneNumber + "/AUTOGEN3/OTP1";

        Request request = new Request.Builder()
                .url(url)
                .get()
                .build();

        try (Response response = okHttpClient.newCall(request).execute()) {
            if (!response.isSuccessful()) {
                throw new RuntimeException("Unexpected response code: " + response);
            }
            return new ResponseEntity<>("OTP send to the registered mobile Number ", HttpStatus.OK);

        } catch (Exception e) {
            e.printStackTrace();
            return new ResponseEntity<>("Failed to send SMS ", HttpStatus.BAD_REQUEST);
        }
    }

    public ResponseEntity<String> verifyOTP(String apiKey, String phoneNumber, String otp) throws Exception {
        OkHttpClient client = new OkHttpClient();
        String api_key = "d5071786-fa68-11ee-8cbb-0200cd936042";
        String url = "https://2factor.in/API/V1/" + api_key + "/SMS/VERIFY3/" + phoneNumber + "/" + otp;

        Request request = new Request.Builder().url(url).get().build();
        try (Response response = client.newCall(request).execute()) {

            if (!response.isSuccessful()) {

                throw new RuntimeException("Unexpected response code : " + response);
            }
            String responseBody = response.body().string();

            return new ResponseEntity<>("Verification result : success", HttpStatus.OK);
        }

    }
}